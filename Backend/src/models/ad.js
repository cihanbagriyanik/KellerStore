...
AdSchema.pre("save", function (next) {
  ...
  if (this.isSold && this.isReserved) {
    return next(
      new Error("An ad cannot be both sold and reserved at the same time.")
    );
  }
...
AdSchema.pre("findOneAndUpdate", async function (next) {
  ...
  if (originalDoc) {
    ...
    if (
      isFavorited &&
      "price" in this._update &&
      this._update.price !== originalDoc.price
    ) {
    ...
    for (const favorite of favorites) {
      ...
      if (user && user.email) {
        sendMail(
          user.email,
          "Fiyat Düstü!",
          "The price of an ad you favorited has dropped!"
        );
      }
    }
  }
      ...
      for (const follower of followers) {
        ...
        if (user && user.email) {
          sendMail(
            user.email,
            "Yeni ürün eklendi!",
            `A user you follow has posted a new ad: ${doc.title}`
          );
        }
      }
    }
